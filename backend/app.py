...
KEPLER_DATASET = "Kepler_DataSet (1).csv"
...
def initialize_system():
    ...
    global model, scaler, all_data, new_data_buffer, current_metrics
...
def process_uploaded_csv(df):
    ...
    if len(df_processed) > 0:
        ...
        if feature_cols_present: df_processed = df_processed.dropna(subset=feature_cols_present, how='all')
    ...
    if (rows_before - len(df_processed)) > 0:
        print(f"⚠ Removed {rows_before - len(df_processed)} rows with at least one missing feature")
    return df_processed
...
def update_model():
    ...
    try:
        ...
        if validation_errors: return jsonify({"error": "Validation failed", "details": validation_errors}), 400
        df_processed = process_uploaded_csv(df)
        if len(df_processed) == 0: return jsonify({"error": "No valid rows after processing"}), 400
...
def get_accuracy():
    ...
    if model is None or len(all_data) == 0: return jsonify({"accuracy": None})
    X = all_data[feature_columns].astype(float); y = all_data['label'].values.astype(int)
    acc = accuracy_score(y, model.predict(scaler.transform(X)))
    return jsonify({"accuracy": round(acc, 4)})
